...
class TestPreprocessing(unittest.TestCase):
    ...
    def test_preprocess_pas(self):
        ...
        database_dict = {'mrn': 478237423, 'dob': datetime(1984, 2, 3), 'sex': 'F', 'last_result': 103.4}
    ...
    def test_preprocess_lims(self):
        ...
        database_dict = {'mrn': 478237423, 'dob': datetime(1984, 2, 3), 'sex': 'F', 'last_result': 99.6}
        expected_age = int((system_dict.get('time') - database_dict.get('dob')).days/365) 
        expected_output = {'age': expected_age, 'sex': 1, 'prev_result': 99.6, 'latest_result': 103.4}
    ...
    def test_preprocess_lims_one_result(self):
        ...
        # LIMS message
        system_dict = {'type': 'LIMS', 'mrn': 478237423, 'time': datetime(2024, 1, 20, 22, 43, 00), 'result': 103.4}
        database_dict = {'mrn': 478237423, 'dob': datetime(1984, 2, 3), 'sex': 'M', 'last_result': None}
        expected_age = int((system_dict.get('time') - database_dict.get('dob')).days/365) 
        expected_output = {'age': expected_age, 'sex': 0, 'prev_result': 103.4, 'latest_result': 103.4}
        self.assertEqual(preprocess(system_dict, database_dict), expected_output)
